...
% for ii = 2:length(field)
%     
%     ral = cat(3, ral, R.(field{ii}));
%     
% end

figure(1)
subplot(321)
imagesc(4 - ral./max(max(ral)).*4)
title('Ground truth cone-beam','FontName','Liberation Serif','FontSize',22)
%caxis([0, 4])
axis image
axis off
%colorbar
...
for jj = 4
    ...
    for noise = 100
        [imcart_2, MSE] = Analysis_DES_linear_plot(cal(:, :, cell2mat(inds(jj))), val(:, :, cell2mat(inds(jj))'), cell2mat(evec(jj)), noise);
...
for jj = 4
    ...
    for noise = 100
        [CNR, MSE] = Analysis_DES_plot_3_1(cal(:, :, cell2mat(inds(jj))), cal(:, :, cell2mat(inds(jj))'), cell2mat(evec(jj)), noise, average_layers(jj));
