function [CNR, perf, noise] = Analysis_DES_plot_3_2(cal, val, evec, layer, varargin)
...
[~, noise] = createContours(sum(val,3),148, 133);
...
for runs = 1:n_runs
    ...
    [ROI_w, SD_w] = createContoursSD(im_hard, 148, 133);
...
if verbose > -1 && global_n == 1
    ...
    subplot(326)
    imagesc(im_hard)
    title('Non-linear pencil-beam','FontName','Liberation Serif','FontSize',22)
    caxis([0, 4])
    axis image
    axis off
    %colorbar
    ...
    figure(1)
    subplot(322)
    imagesc(cat(2,im_hard(:,101:200),im_hard(:,101:200)))
    title('Ground truth pencil-beam','FontName','Liberation Serif','FontSize',22)
    caxis([0, 4])
    axis image
    axis off
    %colorbar
