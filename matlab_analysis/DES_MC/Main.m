...
if linear
    for jj = 1:5 % [1:n_bins-1 8]
        
        n = 1;
        
        for quality = 1:2:n_SNR
            
            cal = squeeze(sum(image_cell(1:quality, :, :, :), 1));
            val = squeeze(sum(image_cell_val(1:quality, :, :, :), 1));
            
            [imcart_2, MSE, noise] = Analysis_DES_linear_3_2(cal, val, cell2mat(evec(jj)));
...
%             [CNR, MSE, noise] = Analysis_DES_3_2(cal, val, cell2mat(evec(jj)), average_layers(jj), ntrial);
            [CNR, MSE, noise] = Analysis_DES_3_2(cal, val, cell2mat(evec(jj)), 11, ntrial);           
            noise = round(noise);
